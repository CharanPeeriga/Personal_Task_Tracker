...
const CALENDAR_COLORS = ['#111111', '#1f2937', '#374151', '#6b7280', '#d1d5db']
...
function GitHubCalendar({ data }) {
  ...
  return (
    <div className="p-5 border border-gray-800/50 rounded-2xl bg-gray-950/60 backdrop-blur-sm overflow-x-auto">
      <div className="flex">
...
function ResolvedProject({ project }) {
  ...
  return (
    <div className="border border-gray-800/50 rounded-xl overflow-hidden bg-gray-950/60 hover:border-gray-700/50 transition-all duration-200">
      <button
        onClick={() => setOpen((v) => !v)}
...
function CompletedTaskRow({ task }) {
  return (
    <div className="flex items-center gap-3 px-4 py-3 rounded-xl border border-gray-800/40 bg-gray-950/40 opacity-70 hover:opacity-100 transition-opacity duration-200">
      <CheckCircle2 className="h-4 w-4 text-green-600 shrink-0" />
      <span className="flex-1 text-sm text-gray-500 line-through truncate">{task.title}</span>
      {task.projectTitle && (
        <span className="text-xs text-gray-600 shrink-0 hidden sm:block">{task.projectTitle}</span>
      )}
      {task.completed_at && (
        ...
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {[
              ...
              { label: 'Active Days', value: calendarData.length, color: 'text-white' },
            ].map(({ label, value, color }) => (
              <div
                key={label}
                className="flex flex-col gap-1 p-4 rounded-xl border border-gray-800/50 bg-gray-950/60 backdrop-blur-sm hover:border-gray-700/50 transition-all duration-200"
              >
          ...
          <section className="flex flex-col gap-3">
            ...
            {resolvedProjects.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-10 text-gray-700 text-sm gap-2 border border-gray-800/40 rounded-xl">
